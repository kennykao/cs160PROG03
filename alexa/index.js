...
function stateHelper(key)
{
    return (session, response, recipe, flow) =>
    {
        ...
        // TODO: handle Flow.prev

        // Next or last direction.
        const nextIndex = parseInt( session.get(Session.index) ) + 1;
        const lastIndex = list.length - 1;

        const index = (flow === Flow.last) ? lastIndex : nextIndex;
        session.set(Session.index, String(index));

        const item = list[index];
        if (index === lastIndex)
        {
            var reply = `The last ${unit} is ${item}. `;
...
app.intent(Intent.ingredients,
    {
        utterances: [
            ...
            "{tell me the|what are the} ingredients"
        ]
    },
    (request, response) =>
    {
